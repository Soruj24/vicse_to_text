    ...
    <Card className="hidden lg:block shadow-xl border-white/5 overflow-hidden rounded-3xl bg-white/40 dark:bg-black/20 backdrop-blur-xl ring-1 ring-black/5 dark:ring-white/10">
      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex justify-between items-center group">
          <span className="text-muted-foreground font-medium group-hover:text-foreground transition-colors">Word Count</span>
          <Badge
            variant="secondary"
            className="px-4 py-1.5 font-mono text-lg bg-primary/10 text-primary border-none rounded-lg group-hover:bg-primary/20 transition-colors"
          >
            {wordCount}
          </Badge>
        </div>
        <div className="flex justify-between items-center group">
          <span className="text-muted-foreground font-medium group-hover:text-foreground transition-colors">Time</span>
          <span className="font-mono font-bold text-xl text-foreground/80 group-hover:text-foreground transition-colors">{formatDuration(recordingDuration)}</span>
        </div>
        {confidence > 0 && (
